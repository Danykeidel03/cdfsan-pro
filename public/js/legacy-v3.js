...
//funciones varias
function arrayOfObjectsToHtmlTableVentasProductosSKU(data, roundNumbers = -1, stripTags = false) {
    var result = '<table><thead>';
    for (var i = 0; i < data.length; i++) {
            ...
            if (object.hasOwnProperty(key)) {
                ...
                if (roundNumbers > -1) {
                    if (cont == 0 || cont == 1 || cont == 2) {
                        tdContent = object[key];
                    } else if (cont == 3 || cont == 4) {
                        var roundTry = parseFloat(object[key]);
                        if (!isNaN(roundTry)) {
                            //tdContent=Math.round(roundTry*1000)/1000;
                            //tdContent=+roundTry.toFixed(2);
                            tdContent = roundTry.toLocaleString('es-ES', {maximumFractionDigits: 0});
                        }
                    } else if (cont == 5) {
                        var roundTry = parseFloat(object[key]);
                        if (!isNaN(roundTry)) {
...
function arrayOfObjectsToHtmlTableVentasProductosBRAND(data, roundNumbers = -1, stripTags = false) {
    var result = '<table><thead>';
    for (var i = 0; i < data.length; i++) {
...
function exportTableToExcel(tableNode, sheetName = '', filename = '', parseNumbers = true) {
...
function exportTableToExcelVentasProducto(tableNode, sheetName = '', filename = '', parseNumbers = true) {
    sheetName = sheetName || 'Hoja 1';
    filename = filename || 'InformePedidos.xlsx';
    //var wb = XLSX.utils.table_to_book(document.getElementById('mytable'), {sheet:sheetName});
    var cortar = false;
